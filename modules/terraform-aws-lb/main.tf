resource "aws_lb" "this" {
  ...
  # checkov:skip=CKV_AWS_91:Logs are not enabled by default
  dynamic "access_logs" {
...
resource "aws_lb_listener_rule" "https_listener_rule" {
  ...
  dynamic "action" {
    ...
    content {
      type             = action.value["type"]
      target_group_arn = aws_lb_target_group.this[lookup(action.value, "target_group_index", count.index)].id
    }
  }
...
resource "aws_lb_listener" "frontend_http_tcp" {
  count = var.enable ? length(var.http_tcp_listeners) : 0

  load_balancer_arn = aws_lb.this[0].arn
  port              = var.http_tcp_listeners[count.index]["port"]
  # checkov:skip=CKV_AWS_2: This block of code is not non TLS traffic
...
resource "aws_lb_listener" "frontend_https" {
  count = var.enable ? length(var.https_listeners) : 0

  load_balancer_arn = aws_lb.this[0].arn

  port = var.https_listeners[count.index]["port"]
  # checkov:skip=CKV_AWS_2: By default this uses HTTPS, but NLB it will be TLS
  ...
  dynamic "default_action" {
    ...
    content {
      type             = "forward"
      target_group_arn = aws_lb_target_group.this[lookup(default_action.value, "target_group_index", count.index)].id
    }
  }
}
